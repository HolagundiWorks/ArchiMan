# AMB Database Migration Backup and Rollback Runbook

## Purpose

This runbook applies to managed AMB deployments. A database backup is required before installing an APK that raises the Room schema version. Measurement rows and managed photo attachments must be treated as one backup set.
...
3. Stop AMB before copying files so the database and WAL are consistent.
...
1. Stop AMB and preserve the failed post-migration data separately for diagnosis.
